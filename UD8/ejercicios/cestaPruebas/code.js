...
class Cesta {
	...
	agregarProducto(producto) {
		...
		if (indice !== -1) {
			//Si el producto ya está en la cesta se le añade
			this.productos[indice] = producto;
		} else {
			//Si es un producto nuevo se agrega a la cesta
	...
	actualizarTotales(total) {
		...
		console.log(`Total + IVA (21%): ${totalConIVA}€`);
	...
	eliminarProducto(producto) {
		...
		if (indice !== -1) {
			//Si el producto está en la cesta se elimina
	...
	mostrarProductos() {
		...
		this.productos.forEach((producto) => {
			...
			botonAñadir.addEventListener("click", () => {
				...
				//Ejecutamos mostrarCestar para actualizar
		...
		//Se muestra la cesta una vez terminada todas las operaciones
		arrayProductos.mostrarCesta();
	}
}
